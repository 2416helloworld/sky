...
核心代码参考：
- 位图切换与事件产出：`src/main/java/com/lk/counter/service/impl/CounterServiceImpl.java:61-75`
- 聚合增量消费与刷写：
  - Kafka 增量入桶：`src/main/java/com/lk/counter/event/CounterAggregationConsumer.java:33-46`
  - 定时刷写到 SDS：`src/main/java/com/lk/counter/event/CounterAggregationConsumer.java:48-104`
- 读路径与异常重建：`src/main/java/com/lk/counter/service/impl/CounterServiceImpl.java:77-125`
- 用户维度异步维护：`src/main/java/com/lk/relation/processor/RelationEventProcessor.java:44-58`
- 灾备全量回放：`src/main/java/com/lk/counter/event/CounterRebuildConsumer.java:35-53`
...
- 批量读取（Feed 场景）：
  ...
  - Feed 汇总与“是否点赞/收藏”判定集成：`src/main/java/com/lk/knowpost/service/impl/KnowPostFeedServiceImpl.java:294-323`。
- 用户维度读取与校验：
...
## 7. 调度与配置
- 调度开关：`@EnableScheduling`（`src/main/java/com/lk/counter/config/CounterConfig.java:14`）。
